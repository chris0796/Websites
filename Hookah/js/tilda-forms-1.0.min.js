...
var $jallforms=$('.r').find('.[data-formactiontype]');if($jallforms.length>0){$jallforms.each(function(){if($(this).data('formactiontype')!=1){$(this).append('<div style="position: absolute; left: -5000px; bottom:0;"><input type="text" name="form-spec-comments" value="Its good" class="js-form-spec-comments"  tabindex="-1" /></div>')}})}
$('.r').find('.js-form-procces').each(function(){try{var formtype=$(this).data('formactiontype');if(formtype==2){$(this).attr('action','#')}}catch(e){console.log(e)}});$('.r').off('submit','.');$('.r').on('submit','.',function(e){var btnformsubmit=$(this).find('[type=submit]');var btnstatus=btnformsubmit.data('form-sending-status');if(btnstatus&&btnstatus==3){btnformsubmit.data('form-sending-status','');return!0}else{if($(this).find('[type=submit]').hasClass('t706__submit_disable')){}else{$(this).find('[type=submit]').trigger('click')}
return!1}});$('.r').on('dblclick','. [type=submit]',function(e){e.preventDefault();return!1});$('.r').off('click','. [type=submit]');$('.r').on('click','. [type=submit]',function(event){event.preventDefault();var btnformsubmit=$(this);var btnstatus=btnformsubmit.data('form-sending-status');if(btnstatus>='1'){return!1}
